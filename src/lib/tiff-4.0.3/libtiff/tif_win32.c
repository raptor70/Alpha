...
// RAPTOR : change name cause blob compilation
static int
_tiffDummyMapProcRaptor(thandle_t fd, void** pbase, toff_t* psize)
{
	(void) fd;
	(void) pbase;
	(void) psize;
...
// RAPTOR : change name cause blob compilation
static void
_tiffDummyUnmapProcRaptor(thandle_t fd, void* base, toff_t size)
{
	(void) fd;
	(void) base;
	(void) size;
...
TIFFFdOpen(thandle_t ifd, const char* name, const char* mode)
{
	...
	tif = TIFFClientOpen(name, mode, (thandle_t)ifd,
			...
			fSuppressMap ? _tiffDummyMapProcRaptor : _tiffMapProc,
			fSuppressMap ? _tiffDummyUnmapProcRaptor : _tiffUnmapProc);
	if (tif)
		tif->tif_fd = ifd;
	return (tif);
}
...
TIFFOpen(const char* name, const char* mode)
{
	...
	tif = TIFFFdOpen((thandle_t)fd, name, mode);
	if(!tif)
		CloseHandle(fd);
	return tif;
}
